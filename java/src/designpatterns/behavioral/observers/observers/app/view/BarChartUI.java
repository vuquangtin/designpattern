...
public class BarChartUI extends javax.swing.JFrame implements CallbackInterface {
	...
	private JLabel getChart() {
		...
		fillData(dataset);
		chart = ChartFactory.createBarChart("Bar Chart", "", "", dataset,
				PlotOrientation.VERTICAL, false, false, false);
		plot = chart.getCategoryPlot();

		renderer = (BarRenderer) plot.getRenderer();
		image = chart.createBufferedImage(this.getWidth(),
				this.getHeight() - 50);
	...
	private void fillData(DefaultCategoryDataset dataset) {
		if (monitors != null) {
			...
			for (ObserverInterface m : monitors) {
				dataset.addValue(Double.parseDouble(m.get().toString()
						.replace(",", ".")), "", (Comparable) m.getName());
				++i;
			}
		}
	}
