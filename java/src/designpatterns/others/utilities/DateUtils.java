...
public class DateUtils {
	...
	public static Date addYears(Date date, int years) {
		if (logger.isDebugEnabled())
			logger.debug("addYears:" + date + " +" + years);
	...
	public static void main(String[] args) {
		...
		Log4jUtils.initLog4j();
		if (logger.isDebugEnabled())
			logger.debug(date.toString() + " add 3 hours:"
		...
		if (logger.isDebugEnabled())
			logger.debug("qq:"
					+ getDifferenceDays(listDatePost.get(0),
							listDatePost.get(1)));
		if (logger.isDebugEnabled())
			logger.debug(DateUtils.addDays(date, 1) + ":"
					+ DateUtils.addDays(date, -11));
		if (logger.isDebugEnabled())
			logger.debug(isAfterNow(DateUtils.addDays(date, 1),
					DateUtils.addDays(date, -11))
					+ "");
		if (logger.isDebugEnabled())
			logger.debug(isBeforeNow(DateUtils.addDays(date, 1),
					DateUtils.addDays(date, -11))
					+ "");
		if (logger.isDebugEnabled())
			logger.debug(DateUtils.addDays(date, 60) + ":"
					+ DateTime.now().toDate());
		if (logger.isDebugEnabled())
			logger.debug(isAfterNow(DateUtils.addDays(date, 60), DateTime.now()
					.toDate())
					+ "");
